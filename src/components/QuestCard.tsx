      ...
      {quest.bonusEffects.length > 0 && (
        <div className="flex items-center mt-2 mb-2">
          {quest.bonusEffects.map((bonus, index) => (
            <div key={index} className="mr-2 bg-game-ui-border bg-opacity-30 rounded-md px-2 py-1 text-game-text flex items-center">
      ...
      {quest.choices && (
        <div className="mt-4 space-y-2">
          {quest.choices.map((choice, index) => (
            <button
              key={index}
              className="w-full text-left bg-game-ui-bg border border-game-ui-border hover:bg-game-ui-border hover:bg-opacity-20 rounded-md px-3 py-2 transition-colors"
